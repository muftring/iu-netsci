...
| Which is not one of the basic ideas of the network community structure?   | **-->** (1) The size of a community should be comparable to the size of the network <br> *Communities do not need to be larger than certain threshold. Almost no method assumes size threshold.* <p><p> (2) Communities should be cohesive (dense), with many internal connections <p> (3) Communities should be separable from the others.  |
| Modularity of a given community measures   | The actual number of internal edges minus the expected number of internal edges.  |
